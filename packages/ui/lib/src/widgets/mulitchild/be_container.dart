...
class BeContainer extends StatelessWidget {
  ...
  Widget build(final BuildContext context) {
    return LayoutBuilder(
      builder: (final context, final constraints) {
        ...
        final effectiveMaxWidth = fluid ? null : maxWidth ?? _getResponsiveMaxWidth(screenWidth);
  ...
  EdgeInsets _getResponsivePadding(final double screenWidth) {
    ...
    return switch (breakpoint) {
      BeBreakpoint.xl2 || BeBreakpoint.xl || BeBreakpoint.lg => const EdgeInsets.symmetric(horizontal: 24.0),
      BeBreakpoint.md => const EdgeInsets.symmetric(horizontal: 20.0),
      BeBreakpoint.sm || BeBreakpoint.xs => const EdgeInsets.symmetric(horizontal: 16.0),
    };
  }
...
extension BeGridUtilsExtension on List<Widget> {
  ...
  List<BeColumn> toResponsiveColumns({
    ...
    return map((final child) => BeColumn(xs: xs, sm: sm, md: md, lg: lg, xl: xl, xl2: xl2, child: child)).toList();
  }
}
...
class BeLayoutTemplates {
  ...
  static List<BeColumn> threeColumn(final Widget left, final Widget center, final Widget right) {
    return [left.beColumn(xs: 12, md: 4), center.beColumn(xs: 12, md: 4), right.beColumn(xs: 12, md: 4)];
  }

  static List<BeColumn> sidebarMain(final Widget sidebar, final Widget mainContent, {final int sidebarSize = 3}) {
    return [sidebar.beColumn(xs: 12, md: sidebarSize), mainContent.beColumn(xs: 12, md: 12 - sidebarSize)];
  }
}
