// This file has been consolidated into be_form_builder_field.dart
// Please use BeFormBuilderField from that location instead.
...
class FormBuilderFieldState<F extends FormBuilderField<T>, T> extends FormFieldState<T> {
  ...
  T? get initialValue =>
      widget.initialValue ?? (_formBuilderState?.initialValue ?? const <String, dynamic>{})[widget.id] as T?;
  ...
  void initState() {
    ...
    _formBuilderState?.registerField(widget.id, this);

    effectiveFocusNode = widget.focusNode ?? FocusNode(debugLabel: widget.id);
  ...
  @override
  void didUpdateWidget(covariant final FormBuilderField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.id != oldWidget.id) {
      _formBuilderState?.unregisterField(oldWidget.id, this);
      _formBuilderState?.registerField(widget.id, this);
    }
    if (widget.focusNode != oldWidget.focusNode) {
      focusAttachment?.detach();
      effectiveFocusNode.removeListener(_touchedHandler);
      effectiveFocusNode = widget.focusNode ?? FocusNode(canRequestFocus: enabled, debugLabel: widget.id);
      effectiveFocusNode.addListener(_touchedHandler);
      focusAttachment = effectiveFocusNode.attach(context);
    }
  }
  ...
  void dispose() {
    ...
    _formBuilderState?.unregisterField(widget.id, this);
    super.dispose();
  }
  ...
  void _informFormForFieldChange() {
    if (_formBuilderState != null) {
      _dirty = true;
      if (enabled || readOnly) {
        _formBuilderState!.setInternalFieldValue<T>(widget.id, value);
        return;
      }
      _formBuilderState!.removeInternalFieldValue(widget.id);
    }
  }
  ...
  bool validate({
    ...
    if (!isValid &&
        ...
        enabled &&
        !fields.values.any((final field) => field.effectiveFocusNode.hasFocus)) {
      focus();
